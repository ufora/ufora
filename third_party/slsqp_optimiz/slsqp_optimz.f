...
C xargs = alpha, f0, gs, h1, h2, h3, h4, t, t0, tol
C iargs = iexact, incons, ireset, itermx, line, n1, n2, n3
...
C iargs = iexact, incons, ireset, itermx, line, n1, n2, n3
C     a different n1 from the n1 declared above!

      DOUBLE PRECISION acc, a(la,n+1), c(la), f, g(n+1),
     *                 x(n), xl(n), xu(n), w(l_w), xargs(10)
...
C   PREPARE DATA FOR CALLING SQPBDY  -  INITIAL ADDRESSES IN W
     ...
     * mode, w(ir), w(il), w(ix), w(im), w(is), w(iu), w(iv), w(iw), 
     * jw, 
     * xargs(1), xargs(2), xargs(3), xargs(4), xargs(5), 
     * xargs(6), xargs(7), xargs(8), xargs(9), xargs(10),
     * iargs(1), iargs(2), iargs(3), iargs(4),
     * iargs(5), iargs(6), iargs(7), iargs(8))
...
C xargs(*) = alpha, f0, gs, h1, h2, h3, h4, t, t0, tol // 10 total
C iargs(*) = iexact, incons, ireset, itermx, line, n1, n2, n3 //8 total. a different n1!!
