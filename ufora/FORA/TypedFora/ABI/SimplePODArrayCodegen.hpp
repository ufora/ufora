...
template<class T>
class NativeTypeForImpl<TypedFora::Abi::SimplePODArray<T> > {
public:
	static NativeType get(void)
		{
		return nativeTypeForCppmlTuple<TypedFora::Abi::SimplePODArray<T> >();
		}
};

template<class T>
class TypedNativeExpressionBehaviors<TypedFora::Abi::SimplePODArray<T>* > {
public:
	...
	TypedNativeExpression<void> resize(TypedNativeExpression<long> inCount) const
		...
		auto lookupFun = 
				makeTypedNativeLibraryFunction(
					&TypedFora::Abi::SimplePODArray<T>::resizeStatic
					);

		return lookupFun(
			TypedNativeExpression<TypedFora::Abi::SimplePODArray<T>*>(mThis),
			inCount
			);
		}
